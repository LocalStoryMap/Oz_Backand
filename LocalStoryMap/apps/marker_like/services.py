...
class MarkerLikeService:
    ...
    def get_like_status(user, marker_id: int):
        ...
        like = MarkerLike.objects.filter(
            user=user, marker=marker, is_liked=True
        ).first()
    ...
    def toggle_like(user, marker_id: int):
        ...
        like, created = MarkerLike.objects.get_or_create(
            user=user, marker=marker, defaults={"is_liked": True}
        )
        ...
        return {"action": action, "like": like, "total_likes": marker.like_count}

    @staticmethod
    def get_marker_likes_list(marker_id: int):
        # 특정 마커의 좋아요 목록 조회
        marker = get_object_or_404(Marker, pk=marker_id)
        return MarkerLike.objects.filter(marker=marker, is_liked=True).select_related(
            "user"
        )
