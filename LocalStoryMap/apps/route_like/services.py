...
class RouteLikeService:
    ...
    def toggle_like(user, route_id: int):
        ...
        like, created = RouteLike.objects.get_or_create(
            user=user, route=route, defaults={"is_liked": True}
        )
        ...
        return {"action": action, "like": like, "total_likes": route.like_count}

    @staticmethod
    def get_route_likes_list(route_id: int):
        # 특정 경로의 좋아요 목록 조회
        route = get_object_or_404(Route, pk=route_id)
        return RouteLike.objects.filter(route=route, is_liked=True).select_related(
            "user"
        )
