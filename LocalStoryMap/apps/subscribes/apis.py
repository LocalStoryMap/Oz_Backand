    ...
    def _verify_payment(self, imp_uid: str, merchant_uid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if payment.get("merchant_uid") != merchant_uid:
                raise IamportAPIError(
                    "merchant_uid가 일치하지 않습니다.", status_code=status.HTTP_400_BAD_REQUEST
                )
            ...
            raise IamportAPIError(
                "결제 검증 중 시간 초과가 발생했습니다.", status_code=status.HTTP_504_GATEWAY_TIMEOUT
            )
    ...
    def post(self, request: Request) -> Response:
        try:
            ...
            payment = self._verify_payment(
                in_ser.validated_data["imp_uid"], in_ser.validated_data["merchant_uid"]
            )
            ...
            return Response(
                {"error": str(e)}, status=e.status_code or status.HTTP_400_BAD_REQUEST
            )
        except Exception as e:
