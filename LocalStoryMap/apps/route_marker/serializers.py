...
class RouteMarkerUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_sequence(self, value):
        ...
        if (
            RouteMarker.objects.filter(route=route, sequence=value)
...
class RouteMarkerBulkUpdateSerializer(serializers.Serializer):
    ...
    markers = serializers.ListField(child=RouteMarkerBulkUpdateItemSerializer())
    ...
    def validate(self, attrs):
        ...
        if RouteMarker.objects.filter(
            id__in=route_marker_ids, route=route
        ).count() != len(route_marker_ids):
